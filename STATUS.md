...
- **T008(再オープン中): `ruling_job.result_json`の旧title(judgeId入り)残存データの移行**: 初回マイグレーション(上記Completed参照)後、Codexレビュー指摘を受け本番DBを読み取り専用クエリで確認したところ複数回にわたり残存(表記揺れ)が発覚し、正規表現ベースの部分置換→完全一致方式(`buildLegacyTitles`、`Set<string>`)への収束、通常移行(`migrateLegacyCorrectionTitlesInResultJson`)は安全側で確定済み(3種類の監査件数`unresolvedRulingJobResultJsonMarkerCount`・`invalidRulingJobResultJsonCount`・`possibleKnownIdCollisionRulingJobResultJsonCount`がすべて0件になることが完了条件)。この方式は**未コミット・未デプロイ**(round1〜18の詳細な経緯は`.ai/tasks/T008-correction-leak-quick-fix.md`のReview History参照)。
  - 完全一致方式では解消されない「explanation文中への全角括弧embedded形式」の既知の残存1件について、round18〜21で自動修復専用スクリプト(部分置換→フィールド値全体の非表示化→検証トークンによるTOCTOU対策)を構築したが、**round22(2026-09-05)でユーザー判断により撤回**: この1件は「バグ稼働期間中に漏洩した可能性のあるIDの再発行は見送り、リスクを受容する」という既存のOut of Scope判断と同種のリスクであり、専用スクリプトの安全性コストが見合わないと判断した。診断(`findUnresolvedLegacyCorrectionTitleJobIds()`、対象jobIdの一覧を返すだけ)と修復(運用者がRender Web Shellから対象jobIdへ直接UPDATE)を分離するシンプルな設計へ変更した。詳細・手動対応手順は`.ai/tasks/T008-correction-leak-quick-fix.md`「方針の再検討・撤回(round22)」参照。
  - `npm run typecheck`・`npm test`(49ファイル/361テスト)PASS。次のステップ: Codexへ実装後レビュー(round22)→問題なければ`CLAUDE.md`の分離コミット→本体コミット→ユーザー承認を得てpush→Renderデプロイ→本番で診断スクリプト実行→手動UPDATE→`migrateCorrectionCredentials.js`再実行で確認
...
- T006: D-004ジャッジ認証強化の対応案(方針決定段階、実装は未着手) — 案A(パスワード追加)/案B(複数ジャッジ合意でhigh昇格制限)/案C(管理者承認ワークフロー)/案D(現状維持+出典表示強化)をドラフトしCodex Review 1完了。当時発見した実装バグ2件(認証不要API`GET /api/corrections/:id`のjudgeIdマスク漏れ、`corrected_by`列への生セッショントークン保存)はT008として先行対応した(上記Completed参照)。**ただしT008は2026-09-04に再オープン中**(`ruling_job.result_json`の旧titleが残存1件、新しい移行コードは未コミット・未デプロイでRender Web Shellでの再実行待ち、詳細は上記In Progress参照)であり、まだClosedではない。残るのはA/B/C/Dの方向性についてのユーザー判断のみ(`.ai/tasks/T006-judge-auth-hardening-proposal.md`、コミット`5f67dc8`)
- **T009(新規、2026-09-04起票、Codex Review 1で原因特定)**: カード名サジェストに同一カード名(「〜輝きは奇跡そのもの〜」)が完全に重複表示される不具合をユーザーが実機で発見。Codex Review 1が公式サイトの公開情報を確認し、原因は現行の異なる2つのカードID(`dm25ex3-002`・`dm25rp3-012`)による正当な同名再録と特定(card_indexの残存レコード清掃が必要という当初仮説は誤りと判明)。モバイル側はidを表示・送信せず名前のみ挿入する仕様のため、サジェストをname単位で重複排除する対応方針で確定。詳細は`.ai/tasks/T009-duplicate-card-suggestion.md`参照。実装は未着手(Review 2実施後)
...
- **T013(新規、2026-09-04起票・実装済み・Codexコードレビュー3回実施済み)**: ユーザー依頼「無償版の場合バナー広告は常に表示に変更、変更箇所は『長押しでカード名を入力』と『質問する』の間」。現状`LoadingBannerAd`は`ruling_screen.dart`・`ruling_turn_view.dart`の2箇所で「裁定生成が処理中の間だけ」表示される設計と判明。`RulingTurnView`は入力欄の無い`RulingJobDetailScreen`とも共用のため、変更範囲はユーザー指定通り`ruling_screen.dart`のみに限定し、質問入力欄とボタンの間に常時表示の広告を追加、同画面内の処理中限定広告は重複表示を避けるため削除した(ユーザー「進めて」指示によりAuto Modeで判断)。Review 1でP1 2件(購読状態未確定のまま広告読み込み→`isStatusKnown`追加/広告の非同期出現によるボタン位置ずれ→高さ固定の`SizedBox`化)、Review 2でP1 1件(RevenueCat問い合わせ失敗時に`isStatusKnown`が確定せず広告が永続的に止まる→try/catch追加)・P2 1件、Review 3(Review 2の修正自体への再レビュー)でP1 1件(Review 2の修正が今度は購読者へも問い合わせ失敗のたびに広告を出してしまう新規不具合を招いていた→`_hasConfirmedOnce`で一度も確定していない場合のみ未購読側へフォールバックし、確定済みなら直前の値を維持するよう修正)を検出・全件反映済み。`flutter analyze` 0 issues。`SubscriptionProvider`単体のFlutterテストは未整備(既知のギャップ)。詳細は`.ai/tasks/T013-free-tier-ad-always-visible.md`参照。未コミット。残る未確定事項(スレッド内追加質問画面への適用要否等)・実機/エミュレータでの視覚確認はユーザー判断待ち/未実施
...
11. ~~T006の公開APIのjudgeIdマスク漏れ・生セッショントークン保存の先行修正要否をユーザーへ確認する~~ → **2026-09-04、T008として先行対応**(上記Completed参照)。**ただしT008は`ruling_job.result_json`の旧titleが残存1件、新しい移行コードは未コミット・未デプロイで再オープン中、Closedではない**(詳細は上記In Progress参照)。残るのはT006の案A/B/C/Dのどの方向で設計を詰めるかの判断のみ(`.ai/tasks/T006-judge-auth-hardening-proposal.md`参照)
