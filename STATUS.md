...
- **T012 Review 8残課題の対応+`scripts/codex-review.ps1`の改修(2026-09-13)**: Vault(`actions/dm-ruling-bot_残作業リスト.md`)側の記録が古く「T009/T010が未実装」と誤って記載されていたが、実際は既に本番反映・確認済み(上記T020参照)と判明。改めて残作業を洗い出し、Claude単独で進められる2件に対応した。
  ...
  - **5回目のレビューラウンド(2026-09-13、「単独で進められるタスクを進めて」指示を受け、4ラウンド目の残課題3件のうち2件に追加対応)**: (a) 検出パターンに環境変数代入形式(大文字スネークケース名+`=`+値)・`Authorization: Bearer`ヘッダー・JSONの`refresh_token`フィールドを追加(完全な網羅は目指さない方針は維持、既知の代表的な形式のみ追加)。(b) `AGENTS.md`/`STATUS.md`/`DECISIONS.md`/タスクファイルにも同じ内容検査(`Test-IsSecretFileContent`)を適用。(c) `git status --porcelain`が空白を含む未追跡パスを引用符付きで返す問題を、`-z`(NUL区切り出力)への変更で解消(`Substring(3)`の代わりにNUL分割+`"?? "`前方一致で正確にパースする方式に変更)。実機(空白・サブディレクトリを含むファイル)で正しく解決できることを確認済み。対応中、新パターン追加の説明コメント自体が検出文字列に反応する自己参照バグを2回発見・再修正し(パターン定義中の`refresh_token`単体語もJSONキー形式`"refresh_token"\s*:`に変更して解消)、Pesterテストを20件に拡充。
  - **5回目のレビューで新たに指摘されたが、実機再現検証の結果「再現しない」と判断し対応しなかった項目**: 「Windows PowerShell 5.1では`>`によるネイティブコマンド出力リダイレクトがUTF-16LEで書き出されるため、UTF-8として復号する現在の実装ではNULが混入し未追跡ファイルが全滅する」という指摘があったが、同一のPowerShell 5.1.26100.9168環境で`[Console]::OutputEncoding`設定を含め全く同じ条件で実際に検証したところ、生バイト列は`EF BB BF 3F 3F 20...`(UTF-8 BOM+正しいASCIIバイト列)であり、UTF-16LEなら生じるはずの交互ゼロバイトパターンは確認されなかった。理論的な懸念であり実機では再現しないと判断し、コード変更は行わなかった(Codexの指摘を機械的に反映せず実際の挙動で検証する、というこのVaultの既存方針に基づく判断)。
  - `npm run typecheck`・`npm test`(373件)・Pester(20件)すべてPASS。コミットは未実施(ユーザーへのpush確認待ち)。
...
- **T013(2026-09-04起票、実装・全レビュー・Androidエミュレータで非購読表示確認完了、購読中表示の実機確認・ストア配信待ち)**: ユーザー依頼「無償版の場合バナー広告は常に表示に変更、変更箇所は『長押しでカード名を入力』と『質問する』の間」。**コミット`a6d2ab2`(2026-09-04、Review 1〜3対応版)は既にmaster/originにpush済み**だったが、STATUS.mdの更新漏れにより「未コミット」と誤記されていたことが判明・訂正。
  ...
  - `flutter analyze` 0 issues。詳細は`.ai/tasks/T013-free-tier-ad-always-visible.md`参照。**コミット`64f7fdf`、push済み(2026-09-05)。リポジトリと記録済みリリース履歴上、現行ストア版`1.7.2+18`より実装が後で未配信と判定。次回ストア配信後の購読中実機確認が必要**
...
13. **T012旧孤立ジョブ1件の手動返金要否を判断する**。自動返金なしは確認済みだが、当時無料枠を消費したかの確定には当該deviceIdと当時の購読・利用状態を個別調査する必要がある。本番データ更新は別途ユーザー判断後に行う
14. **T013を含む次回Android/iOSストアリリース後、購読中実機で広告非表示とフォーム間隔を確認する**
