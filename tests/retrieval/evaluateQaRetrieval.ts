...
import { searchAndRankQa } from "../../src/rules/qaRanking";
import { semanticSearchQa } from "../../src/search/semanticSearch";
import { hybridSearchQa } from "../../src/search/hybridSearch";
...
async function rankedIdsForMode(
  ...
  if (mode === "keyword") {
    const results = await searchAndRankQa(criteria, { topN: CANDIDATE_COUNT });
    return results.map((r) => r.id);
  }
  if (mode === "embedding") {
    const results = await semanticSearchQa(question, CANDIDATE_COUNT);
    return results.map((r) => r.id);
  }
  const results = await hybridSearchQa(question, criteria, { finalResultCount: CANDIDATE_COUNT });
  return results.map((r) => r.id);
}
  ...
  for (const testCase of cases) {
    ...
    for (const mode of MODES) {
      ...
      for (const k of CUTOFFS) {
        if (hitsAtK(rankedIds, testCase.expectedQaIds, k)) recallHits[k]! += 1;
      }
      mrrSumByMode.set(mode, mrrSumByMode.get(mode)! + reciprocalRank(rankedIds, testCase.expectedQaIds));
    }
  }
...
async function main(): Promise<void> {
  const casesPath = join(__dirname, "qa_cases.json");
