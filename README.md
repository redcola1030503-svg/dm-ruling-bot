...
**ジャッジ/管理者の管理について**: `VALID_JUDGE_IDS`(ジャッジ)・`ADMIN_JUDGE_IDS`(管理者)環境変数は、DBが空の初回起動時にのみ`judge`テーブルへシードされる初期値です。以後の追加・削除はDBで管理され、`/judge_add`・`/judge_remove`コマンド(管理者のみ)で行います。既存のジャッジIDを管理者に昇格させたい場合は、一度`/judge_remove`で削除してから、管理者経由で再度登録し直すのではなく、DBを直接操作するか、`ADMIN_JUDGE_IDS`にIDを追加した状態でDBを作り直す必要があります(現時点でロール変更コマンドは未実装)。
