...
- [x] カード名あいまい時のWeb検索自動確定 — カード名が公式データベース上で複数候補に割れ確定できない場合、Claude APIのWeb検索ツールで現在の対戦環境での話題度を調査し、候補の1つが圧倒的に優勢と判断できた場合のみ自動確定する。判断できない場合は従来通りユーザーへ候補確認を返す(実機確認済み)
...
質問文から抽出したカード名が公式カード検索で一意に確定できない場合(例:「ベートーベン」→「ベートーベン・キューブ」「「修羅」の頂 VAN・ベートーベン」等が並立)、誤ったカードを前提に裁定を生成してしまうリスクを避けるため、原則としてLLMには回さずユーザーへ候補を提示して確認します。

これに加えて、候補のうち1つが現在の対戦環境で**圧倒的に**多く使用・話題になっている場合は、Claude APIのWeb検索ツール(`web_search_20260209`)で調査し、自動的にそのカードとして確定します(`src/cards/resolveAmbiguousCardByPopularity.ts`)。判断が僅差・不明な場合は無理に確定させず、従来通り候補確認にフォールバックします。
...
| `ENABLE_POPULARITY_CARD_RESOLUTION` | 任意(既定`true`)。カード名あいまい時のWeb検索自動確定機能のON/OFF。Web検索利用によりコストが発生するため、運用状況に応じて`false`で無効化できる |
