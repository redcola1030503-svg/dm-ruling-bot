...
import { getAllQaIndexChunkRows, saveQaEmbedding } from "../rules/qaIndexRepository";
import { VoyageEmbeddingProvider, isEmbeddingSearchConfigured } from "../embeddings/VoyageEmbeddingProvider";

const BATCH_SIZE = 100;

function toEmbeddingText(row: { question: string; answer: string }): string {
...
async function main(): Promise<void> {
  ...
  const rows = getAllQaIndexChunkRows();
  ...
  console.log(`対象Q&A数: ${rows.length}`);
  ...
  for (let i = 0; i < targets.length; i += BATCH_SIZE) {
    ...
    try {
      const embeddings = await provider.embedDocuments(batch.map((row) => toEmbeddingText(row)));
      ...
      logger.error("qa_embedding_batch_failed", {
        batchStart: i,
