 ...
 * 通知テスト送信。
 *
 * このエンドポイントを叩く「通知テストを送信」ボタンは、モバイルアプリ側から
 * 廃止済み(v1.6.2以降)。Androidはアプリがフォアグラウンドにある間はFCMの通知を
 * 自動表示せず、アプリ側にonMessageハンドラも無いため、設定画面(=常に
 * フォアグラウンド)から押すこのボタンは構造的に通知を表示できなかったため。
 *
 * ただし既に配信済みの旧バージョン(v1.4.0〜v1.6.1)にはまだボタンが残っており、
 * ここを削除するとそれらがエラー表示になるため、当面は残しておく。
 * 旧バージョンが十分に入れ替わったら削除してよい。
 */
