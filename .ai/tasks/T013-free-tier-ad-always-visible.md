...
Status: 実装・Codexコードレビュー・Androidエミュレータでの非購読表示確認完了。コミット`a6d2ab2`・`64f7fdf`はpush済み。リポジトリと記録済みリリース履歴上、本番ストア版には未配信。購読中表示の実機確認は次回ストアリリース後も残る

## 本番確認可否（2026-09-06、T020）

リポジトリの現行モバイルバージョンは`1.7.2+18`で、そのバージョン更新コミット`a0da73a`は2026-09-01。`STATUS.md`でも同ビルドを両OSへ配信した記録が最新である。一方、T013の実装コミットは`a6d2ab2`（2026-09-04）・`64f7fdf`（2026-09-05）であり、その後のバージョン更新・ストア配信記録はない。このリポジトリと記録済みリリース履歴を根拠に、現行ストア版にT013は含まれないと判定した（今回ストアコンソール自体は再確認していない）。そのため購読中の広告非表示を本番ストア版で確認することは現時点では不可能。次回Android/iOSストアリリース後、実際の購読中端末で広告が表示されないことと`collapsedHeight`分のフォーム間隔を目視確認する。
