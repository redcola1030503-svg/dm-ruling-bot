...
Status: 実装・本番回収確認完了。コミット`d117242`はpush済み。既存旧ジョブ1件の手動返金要否は未判定

## 本番確認（2026-09-06、T020）

Render Web Shellから本番DBを`readOnly: true`で開き、個別のjobId・deviceId・質問・回答を取得しない集計SELECTを実行した。2026-09-01 JST作成、確定まで24時間超、`failed`という既存記録の孤立ジョブXに合致する行は1件だけで、`finished_at`あり、`outcome_status`なし、`worker_id`/`heartbeat_at`なしだった。これにより、デプロイ後の孤立ジョブ回収が本番で実際に作動したことを確認した。

この旧ジョブは`usage_month_key`がなく、`refunded_at`もないため、自動返金されていない。これはT010/T012導入前のジョブへ消費月を安全に遡及できない既知の設計どおりである。今回の非識別集計だけでは当時無料枠を消費したか断定できず、本番データの更新を伴う手動返金は行っていない。返金要否を確定するには当該deviceIdと当時の消費・購読状態を個別に再調査する必要があり、本タスクの残作業とする。最初のコマンドはWeb Shellの引用符解釈によりDB接続前の構文エラーとなったため、同じSELECTを数値時刻境界とSQLパラメータへ置き換えて再実行し成功した。
