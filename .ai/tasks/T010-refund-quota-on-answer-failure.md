...
Status: Complete。実装・Codexコードレビュー・commit/push完了(コミット`d117242`)。本番のライブコミット`de8ed26`に含まれ、本番結果が返金実装と整合することを2026-09-06に確認済み（消費直後の9件は未観測）

## 本番確認（2026-09-06、T020）

検証専用のランダムな合成deviceIdを使用し、個別IDは記録していない。POST前に本番サジェストで「ベートーベン」が6候補になることを確認した。初期無料枠10件を確認後、検証用質問を1件だけ送信し、4回目のポーリングで`status=done`・`outcomeStatus=needs_clarification`を受信した。完了後の無料枠は10件で、非購読・質問可能の状態も維持された。送信直後の9件は観測していないため、この本番確認単独では「消費後に返金」と「消費されなかった」を区別できない。一方、`POST /api/ruling/jobs`は無料枠加算をジョブ作成と同一の同期トランザクションで完了してから202を返す実装であるため、202受信済み・返金対象結果・最終10件という本番結果は返金実装と整合する。合成ジョブ1件は本番DBへ残るため、将来の利用件数分析ではT020の検証データ1件が含まれる点に留意する。
